...
Function Get-TwitterFollowers {
	[CmdletBinding(DefaultParameterSetName='GetByName')]
	[OutputType('System.Management.Automation.PSCustomObject')]
	param (
		[Parameter(Mandatory,ParameterSetName='GetByName')]
        [ValidateNotNullOrEmpty()]
		[string]
        $ScreenName
        ,
	...
	Process {
		$HttpEndPoint = 'https://api.twitter.com/1.1/friends/ids.json'
		$ApiParams = @{
            'count' = $Count
            'stringify_ids' = $StringifyIds
		}
        if ($PSCmdlet.ParameterSetName -ieq 'GetByName') {$ApiParams.Add('screen_name', $ScreenName)}
        if ($PSCmdlet.ParameterSetName -ieq 'GetById')   {$ApiParams.Add('user_id', $UserId)}
		$AuthorizationString = Get-OAuthAuthorization -Api Timeline -HttpEndPoint $HttpEndPoint -ApiParameters $ApiParams -HttpVerb Get
...
#New-MyTwitterConfiguration -Force -APIKey 'TQJLvGa62kftjhxxTM9ddrShs' -APISecret '4AVYxRDzHAd04L6aNDn3qUM8uX0TFGEHH5iSAmt1BBPgxgTdWo' -AccessToken '76157528-QO6cSC9gNoGnKfQhGFPMOieprHK2ppCBSVEfQfnaD' -AccessTokenSecret 'uhvdKOIn7oEiIgSqMt5PsOAnRYOegFaIos9UVco978xln'
...
#$response = Get-TwitterRetweets -Id '743819108156841984'
#$content = $response | ConvertFrom-Json
#$content.user.id

Get-TwitterFollowers -ScreenName 'NicholasDille'